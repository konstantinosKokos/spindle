...
DecoderItem = list[list[list[list[tuple[int, int]]]]]


def batchify_decoder_inputs(trees: list[DecoderItem]) -> Iterator[tuple[Tensor, Tensor, Tensor]]:
    concatenated = sum(trees, [])
    depths = zip_longest(*[t for t in concatenated], fillvalue=[])
    for depth in depths:
        root_ids, symbols, positions = zip(*((root_id, symbol, hotbit)
                                             for root_id, nodes in enumerate(depth)
                                             for symbol, hotbit in nodes))
...
def batchify_encoder_inputs(token_ids: list[Tensor],
                            ...
                            pad_token_id: int,
                            cls_dist: int = -100) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
...
Batch = tuple[Tensor, Tensor, Tensor, Tensor, Tensor, list[Tensor], list[Tensor], list[Tensor]]
...
def collate_fn(batch_items: BatchItems,
    ...
    root_to_node_index, node_ids, node_pos = list(zip(*batchify_decoder_inputs(decoder_inputs)))
